    ...
    public partial class report : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                switch (report)
                {
                    case "montosPendientesCobroVencimientos":
                        {
                            errorMessage = "";

                            if (!montosPendientesCobroVencimientos_consulta_report(database, userID, out errorMessage))
                            {
        ...
        private bool montosPendientesCobroVencimientos_consulta_report(IMongoDatabase database, string userID, out string errorMessage)
        {
            errorMessage = "";

            var collection = database.GetCollection<BsonDocument>("consulta_MontosPendientesCobro_Vencimientos");
            var collection_config = database.GetCollection<BsonDocument>("consulta_MontosPendientesCobro_Vencimientos_config");
            ...
            List<montoPendienteCobro_item> reportItems = new List<montoPendienteCobro_item>();
            montoPendienteCobro_item reportItem;
            ...
            foreach (BsonDocument document in cursor.ToEnumerable())
            {
                ...
                reportItem = new montoPendienteCobro_item
                {
                    ...
                    montoPorPagar = Convert.ToDecimal(document["montoPorPagar"]),
                    saldo1 = Convert.ToDecimal(document["saldo1"]),

                    montoCobrado = Convert.ToDecimal(document["montoCobrado"]),
                    montoPagado = Convert.ToDecimal(document["montoPagado"]),
                    saldo2 = Convert.ToDecimal(document["saldo2"])
                };
            ...
            if (formatoExcel.HasValue && formatoExcel.Value)
            {
                ...
                ReportViewer1.LocalReport.ReportPath = "reports/consultas/montosPendientesCobroVencimientos/report-excel.rdlc";
            } else
            {
                ReportViewer1.LocalReport.ReportPath = "reports/consultas/montosPendientesCobroVencimientos/report.rdlc";
            }
